...
class AddLocationForm(BoxLayout):
    ...
    def search_location(self):
        search_template = 'http://api.openweathermap.org/data/2.5/' + "weather?q={}&type=like&units=metric&appid=6f23dc440fdfe6984e278469c981cf69"
        search_url = search_template.format(self.search_input.text)
        request = UrlRequest(search_url, self.found_location)

    def found_location(self, request, data):
        data = json.loads(data.decode()) if not isinstance(data, dict) else data
        cities = [(data['name'], data['sys']['country'])]
...
class CurrentWeather(BoxLayout):
    ...
    def update_weather(self):
        weather_template = 'http://api.openweathermap.org/data/2.5/' + "weather?q={},{}&type=like&units=metric&appid=6f23dc440fdfe6984e278469c981cf69"
